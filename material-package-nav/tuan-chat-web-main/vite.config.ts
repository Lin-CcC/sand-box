...
export default defineConfig(({ command, mode }) => {
  ...
  return {
    ...
    server: {
      ...
      host: "0.0.0.0",
      proxy: {
        // 说明：项目里存在 `api/*` 源码目录（OpenAPI 生成代码），Vite 运行时会请求 `/api/*.ts(x)`。
        // 因此不要把 `/api` 作为代理前缀，否则会把模块请求代理到后端导致页面加载失败。
        //
        // 推荐：把 `VITE_API_BASE_URL` 设为 `/__api/api`，并在此处把 `/__api` 代理到后端域名，避免跨域 CORS。
        "/__api": {
          target: env.VITE_API_PROXY_TARGET || "https://test.tuan.chat",
          changeOrigin: true,
          secure: true,
