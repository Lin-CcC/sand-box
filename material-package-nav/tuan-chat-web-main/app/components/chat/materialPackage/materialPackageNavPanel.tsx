...
export default function MaterialPackageNavPanel({
  ...
  const [toolbarPinned, setToolbarPinned] = useLocalStorage<boolean>("tc:material-package:explorer-toolbar-pinned", true);
      ...
      >
        <div className="p-3 space-y-3">
          ...
          <div className="space-y-2">
            {/*
             * 工具栏显示规则：
             * - 默认锁定（toolbarPinned=true）：按钮常显（方便移动端）
             * - 未锁定：桌面端 hover 显示；触摸端可按住临时显示
             * - 点击 TUAN-CHAT 文本：切换锁定/隐藏
             */}
            <div
              className="px-1 flex items-center justify-between gap-2"
              onPointerEnter={() => {
                if (suppressHoverUntilLeaveRef.current)
                  return;
                setIsToolbarHover(true);
              }}
              onPointerMove={() => {
                if (suppressHoverUntilLeaveRef.current)
                  return;
            ...
            >
              <button
                type="button"
                className="text-[11px] font-semibold tracking-wider text-base-content/50 hover:text-base-content/70 active:text-base-content/80 transition-colors"
                title={toolbarPinned ? "点击：隐藏工具栏（改为悬浮显示）" : "点击：锁定显示工具栏"}
                onClick={() => {
                  if (toolbarPinned) {
                    // 从“锁定常显”切换到“悬浮显示”时，用户期望立即隐藏。
                    // 由于鼠标此时仍在该行上，需要抑制 hover，直到指针离开后再恢复。
              ...
              <div
                className={`flex items-center gap-1 transition-opacity ${
                  (toolbarPinned || isToolbarHover) ? "opacity-100 pointer-events-auto" : "opacity-0 pointer-events-none"
                }`}
              >
