...
export default function MaterialPackagePage() {
  ...
  const [activePreview, setActivePreview] = useState<ReturnType<typeof getMaterialPreviewDragData>>(null);
  ...
  const dockPreview = useCallback((payload: ReturnType<typeof getMaterialPreviewDragData>, options?: { index?: number; placement?: "top" | "bottom" }) => {
    if (!payload)
      return;
        ...
        <ChatPageLayout
          ...
          sidePanelContent={(
            <MaterialPackageNavPanel
              ...
              onOpenPreview={(payload, hintPosition) => openPreview(payload, hintPosition)}
            />
          )}
